...
pub fn render(c: &mut Canvas, t: &Theme, f: &FontSet, found: usize, pairing: Option<usize>) {
    ...
    text::draw(c, f, 76.0, (cy0 + 30) as f32, "One-touch NFC", &sty(Family::Sans, Weight::SemiBold, 14.0, t.ink, 0.0));
    text::draw(c, f, 76.0, (cy0 + 50) as f32, "Touch an NFC device to the rear panel to pair", &sty(Family::Sans, Weight::Regular, 11.0, t.dim, 0.0));

    // DISCOVERABLE · n
    let mut y = cy0 + ch + 18;
    text::draw(c, f, 22.0, (y + 14) as f32, &format!("DISCOVERABLE · {}", found), &sty(Family::Mono, Weight::Regular, 9.0, t.faint, 0.18));
    y += 28;

    if found == 0 {
        bars(c, 160, y + 16, 160, 20, 16, 4, 9.0, t.acc, t.line);
        center(c, f, 240.0, (y + 58) as f32, "LISTENING FOR DEVICES…", &sty(Family::Mono, Weight::Regular, 10.0, t.faint, 0.1));
    } else {
        let rh = 62;
        for d in DISCOVERABLE.iter().take(found) {
            let cy = y + rh / 2;
            icons::bt(c, 34.0, cy as f32, 16.0, t.dim);
            text::draw(c, f, 58.0, (cy - 2) as f32, d.name, &sty(Family::Sans, Weight::SemiBold, 15.0, t.ink, 0.0));
    ...
    // tip footer
    hline(c, 736, t.line);
    text::draw(c, f, 22.0, 760.0, "TIP: HOLD YOUR HEADPHONES' POWER BUTTON ~7s", &sty(Family::Mono, Weight::Regular, 9.0, t.faint, 0.1));
    text::draw(c, f, 22.0, 776.0, "UNTIL THE LED BLINKS BLUE.", &sty(Family::Mono, Weight::Regular, 9.0, t.faint, 0.1));
}
