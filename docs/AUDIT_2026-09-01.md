...
> **2026-09-11 — the 1.3 GB was one working copy's `.git`, not the clone.** GitHub reports the
> repository at 124 MB, and a fresh clone downloads about that; the `.git` that was measured held
> 1.09 GB of loose objects that `git gc` had never packed. The committed binaries are still most of
> the history, and option 3 is still how to drop them — now as one step of a rewrite that also takes
> Sony's files out, rehearsed in [`HISTORY_REWRITE.md`](HISTORY_REWRITE.md).
