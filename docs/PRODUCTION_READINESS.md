...
| Sony's `AudioAnalyzerService` emits frames — `cinder-probe --analyzer` has **never been run** | `viz_analyzer_tick` | the visualiser never draws at all; the synthetic fallback was deliberately removed |
