...
| 13.11 | **Mono on the 3.5 mm jack — the one unswept lever.** NOT a bug to fix: §1–5 of `analysis/RE_mono_audio.md` closes every other route, and §5 is closed by SECURITY.md rule 1 (never write a codec register). This item exists only to settle §7 by measurement instead of leaving it open | With the bench rig from `analysis/RE_headphone_amp_modes.md` (1 kHz hard-panned tone, capture at the jack), sweep `Audio I2sout Mch Config` through 0..6 with `amixer -c0`, capturing both channels at each. It is an MTK-side mixer control, not a codec register, and a mixer value does not survive a reboot — so this is reversible. Restore `5,6` afterwards | Either one value produces L and R carrying the same summed signal — in which case mono reaches the jack and the Sound row can stop qualifying itself — or none does, and §7 is closed | Output silent or mis-routed at some values: expected, that is what a sweep is for. Reboot restores it. **Do not** extend this to codec registers |
