...
| 13.7 | **Lyrics show and follow the song** — new; the reference library has no `.lrc` to test with | Over USB, put a timestamped `Song.lrc` beside a song with the same name; play it; tap the title block on Now Playing ▸ Lyrics | A `Lyrics · Synced, N lines` row on Track information; the page highlights the sung line and keeps it in view; a drag stops the following; the next song, without a `.lrc`, has no Lyrics row | No row: the names must match apart from the extension (`.lrc` / `.LRC`). Garbled words: note the file's encoding |
