...
| 13.9 | **The WM1A volume curve from a table the user brings** — new; the reference device had it copied in over adb, not by an install | (a) Wampy installed, nothing on the drive: install with `wm1a`. (b) A player without Wampy: put `ov_127x.tbl` and `ov_dsd_127x.tbl` from the cinder-sony-analysis release at the top of the drive, install with `wm1a`. (c) Any other file renamed `ov_127x.tbl` on the drive | Install log `volume table: installed ov_127x.tbl from … (SHA-256 matches Sony's)`; boot log `volume curve: wm1a applied`; `cinder-probe --volcurve` monotonic (vol 40 → PHV 84, vol 60 → 124). (c): `WARN: /contents/ov_127x.tbl is not Sony's ov_127x.tbl` and the stock curve | Boot log `needs Sony's ov_127x.tbl`: nothing reached `/system/vendor/unknown321/usr/share/cinder/audio_dac` — read the install log's `volume table:` and `WARN:` lines. A browser may have renamed the download (`ov_127x (1).tbl`) |
